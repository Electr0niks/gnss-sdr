/*!
 * \file sbas_l1_ca_dll_pll_tracking.cc
...
SbasL1CaDllPllTracking::SbasL1CaDllPllTracking(
      ...
      out_streams_(out_streams)
{
    ...
    // SBAS L1 uses the same chip rate and code length as GPS L1 C/A
    ...
    if (trk_params.extend_correlation_symbols < 1)
        {
            ...
            std::cout << TEXT_RED << "WARNING: SBAS L1 C/A. extend_correlation_symbols must be bigger than 1. Coherent integration has been set to 1 symbol (1 ms)" << TEXT_RESET << '\n';
        }
    else if (trk_params.extend_correlation_symbols > 20)
        {
            trk_params.extend_correlation_symbols = 20;
            std::cout << TEXT_RED << "WARNING: SBAS L1 C/A. extend_correlation_symbols must be lower than 21. Coherent integration has been set to 20 symbols (20 ms)" << TEXT_RESET << '\n';
        }
    ...
    if (trk_params.track_pilot)
        {
            ...
            std::cout << TEXT_RED << "WARNING: SBAS L1 C/A does not have pilot signal. Data tracking has been enabled" << TEXT_RESET << '\n';
        }
    if ((trk_params.extend_correlation_symbols > 1) and (trk_params.pll_bw_narrow_hz > trk_params.pll_bw_hz or trk_params.dll_bw_narrow_hz > trk_params.dll_bw_hz))
        {
            std::cout << TEXT_RED << "WARNING: SBAS L1 C/A. PLL or DLL narrow tracking bandwidth is higher than wide tracking one" << TEXT_RESET << '\n';
        }
...
void SbasL1CaDllPllTracking::stop_tracking()
{
    tracking_sptr_->stop_tracking();
}


void SbasL1CaDllPllTracking::start_tracking()
{
    tracking_sptr_->start_tracking();
}


void SbasL1CaDllPllTracking::set_channel(unsigned int channel)
{
    channel_ = channel;
...
void SbasL1CaDllPllTracking::set_gnss_synchro(Gnss_Synchro* p_gnss_synchro)
{
    tracking_sptr_->set_gnss_synchro(p_gnss_synchro);
}


void SbasL1CaDllPllTracking::connect(gr::top_block_sptr top_block)
{
    if (top_block)
...
void SbasL1CaDllPllTracking::disconnect(gr::top_block_sptr top_block)
{
    if (top_block)
        { /* top_block is not null */
        };
...
gr::basic_block_sptr SbasL1CaDllPllTracking::get_left_block()
{
    return tracking_sptr_;
}


gr::basic_block_sptr SbasL1CaDllPllTracking::get_right_block()
{
    return tracking_sptr_;
}
